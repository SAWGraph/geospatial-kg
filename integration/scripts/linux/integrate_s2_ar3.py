...
def integrate_s2(states, source, wd, input_path, name):
    ...
    print('   Integrating\n')
    try:
        command = fr'''/opt/ohpc/pub/apptainer/1.2.5/bin/apptainer exec --cleanenv --bind "/scratch/kedrowski/s2-ar3_integration:/s2:rw" /scratch/graphdb/sawgraph-tools/s2-coverings-main.sif bash -c 'cd /s2 && python3 s2-coverings/src/integrate.py --path "{input_path}"' '''
        subprocess.run(['bash', '-l', '-c', command])
...
def integrate_ar3(states, source, wd, input_path, dim, outfile):
    ...
    print('   Integrating\n')
    try:
        command = f'python3 xintegrate.py {input_path} {dim} {wd} 2 {outfile}'
        subprocess.run(['bash', '-l', '-c', command], check=True, capture_output=True, text=True)
    except Exception as e:
        print(f'Exception: {e}')
